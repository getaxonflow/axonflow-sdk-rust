...
## [0.11.0] - 2026-09-14: decision provenance, the PEP capability handshake, and typed policy authoring
...
- **Two runnable v11 examples, in the order to run them.**
  ...
  `AXONFLOW_TYPED_POLICY_PUBLISH=1`, printing the publish response's
  template-omission report, and exits non-zero when a publication or
  activation it asked for is refused. Run the handshake example first: after a
  document with an organization-scope constraint is activated, a decide that
  ...
  first reason is that code, followed by one naming each constraint it could
  not evaluate and the attribute it needed
...
- **`AUTHZEN_PATH` and `AUTHZEN_PROFILE_HEADER` are generated from the surface
  ...
  constants; the `authzen` module re-exports them through `pub use types_gen::*` under the names it always had.
...
- **No migration is required.** A 0.8.2 integration behaves identically on
...
- The mapping is mechanical: `stage: "llm"` becomes
  ...
  `stage: "agent"` becomes `agent.invoke`. `query` moves to
  `context.args.query` (`AuthZenRequest::with_query`),
  `caller_identity.gateway_id` becomes `subject.id` with
  `subject.type = "gateway"`, and `verdict` becomes the `decision` boolean plus
  `context.state`, both of which `AuthZenDecision::allowed()` requires.
- **The one behavioural difference to plan for**: the legacy surface accepted
  ...
  (`Err(AuthZenEvaluationError::Refused(..))` versus `Ok(decision)` whose
  `allowed()` is false), so a caller that branched on a boolean needs one more
  arm.
