...
- **Which edition refuses what.** From v11.0.0, on every edition, `decide` refuses with `unsupported_obligation` a mandatory obligation the enforcement point cannot discharge (an organization's redact override is the shipped case). It judges against the declared capabilities, so on Community too, a caller that declares `field_mask` but not `field_redact` is refused under a redact override. On Enterprise, in addition, any allow carrying a mandatory obligation outside the declared set becomes a deny. A Community deployment drops a declared capability in a family its edition does not issue, counts it, and lets the request proceed.
